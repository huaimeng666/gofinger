...
- **精准主动指紋探测**：通过 `-a` 参数启用，由 `finger.yaml` 驱动，实现“路径-规则”一对一的精确匹配，增加主动识别功能，对目标进行二次路径探测，发现隐藏得更深的指纹信息。发现隐藏资产，极大降低漏报误报率。
...
- **Fofa URL处理优化**：彻底解决了 Fofa 结果中 URL 端口重复的问题，并确保在处理 IPv6 地址时（例如 `[::1]:8080`）也能正确识别和拼接 URL，避免因冒号而导致的误判。（v0.8）
...
- **界面优化**：**优化CMS、SERVER、TITLE 颜色显示 （v0.7）**

  ![wechat_2025-09-09_142150_765](img/wechat_2025-09-09_142150_765.png)
...
**请求优化**:                                                                          
对于主动指识别库中被多条规则引用的同一个路径（例如，一条规则匹配标题，另一条规则匹配正文），程序足够智能将包含“一次请求，多次匹配”的逻辑，以解决重复请求的问题。，**只会发送一次网络请求**，避免了不必要的性能开销。

   1. 收集并合并路径：在主动扫描开始时，程序会先遍历 finger.yaml 中的所有规则，提取出所有 path，并创建一个“路径
       -> [规则列表]”的映射表。对于重复的路径（如
      /nacos/），它会将所有指向该路径的规则（title规则、body规则等）都收集到一个列表中。

   2. 按路径探测：接下来，程序会遍历这个独一无二的路径列表。

   3. 一次请求，多次匹配：对于每个路径，程序只会发送一次网络请求。拿到响应后，再用这个响应去逐一匹配之前收集到
      的、所有与该路径关联的规则。

### **新增：精准主动指纹探测(v0.6)**

- 推出全新的主动扫描 (`-a`) 功能，其逻辑和规则源完全重写。
- **废除 `FingerDir.yaml`**，主动扫描现在由 `finger.yaml` 文件驱动。
